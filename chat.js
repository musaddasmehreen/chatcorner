...
let privateChats = new Map(); // userId -> { unread: bool }
...
async function sendMessageToSupabase(messageText, avatarUrl) {
    ...
    try {
        console.log("[DATABASE] Sending message...");
        const { data, error } = await supabase
            .from("messages")
            .insert([{ room_id: currentRoom, username: currentUsername, message_text: messageText, avatar_url: avatarUrl, created_at: new Date().toISOString() }])
            .select();
        if (error) throw error;
        console.log("[DATABASE] Message inserted:", data);
        return true;
    } catch (error) {
...
async function sendVoiceNoteToSupabase(dataUrl) {
    ...
    try {
        console.log("[DATABASE] Sending voice note...");
        const { data, error } = await supabase
            .from("messages")
            .insert([{ room_id: currentRoom, username: currentUsername, message_text: "[Voice Note]", voice_url: dataUrl, avatar_url: currentAvatarUrl, created_at: new Date().toISOString() }])
            .select();
        if (error) throw error;
        console.log("[DATABASE] Voice note inserted:", data);
        return true;
    } catch (error) {
        console.error("[DATABASE ERROR]", error);
...
async function fetchMessageHistory() {
    try {
        ...
        const { data, error } = await supabase.from("messages").select("*").eq("room_id", currentRoom).order("created_at", { ascending: true }).limit(50);
        if (error) throw error;
...
function openPrivateChat(userId, username) {
    ...
    const tabBar = document.getElementById("privateUsersTabBar");
    if (!document.querySelector(`[data-user-id="${userId}"]`)) {
        const tab = document.createElement("div");
        tab.className = "private-user-tab active";
        tab.dataset.userId = userId;
        tab.innerHTML = `
            <span class="private-user-tab-name">${username}</span>
            <button class="private-user-close-btn" onclick="closePrivateChat('${userId}')">×</button>
            <div class="unread-indicator" style="display:none;"></div>
        `;
        tab.onclick = () => switchPrivateChat(userId, username);
        tabBar.appendChild(tab);
        privateChats.set(userId, { unread: false });
    }
    ...
    const indicator = document.querySelector(`[data-user-id="${userId}"] .unread-indicator`);
    if (indicator) indicator.style.display = "none";
    
    // Update active tab
    document.querySelectorAll(".private-user-tab").forEach(t => t.classList.remove("active"));
    document.querySelector(`[data-user-id="${userId}"]`).classList.add("active");
    
    // Load chat history
    fetchMessageHistory();
    
    console.log("[PRIVATE CHAT] Opened with", username);
}
...
function closePrivateChat(userId) {
    ...
    if (Object.keys(Object.fromEntries(privateChats)).length === 0) {
        document.getElementById("privateUsersTabBar").classList.add("hidden");
    }
...
function switchPrivateChat(userId, username) {
    openPrivateChat(userId, username);
}

function markPrivateChatUnread(userId) {
    const indicator = document.querySelector(`[data-user-id="${userId}"] .unread-indicator`);
    if (indicator) indicator.style.display = "block";
    if (privateChats.has(userId)) {
        privateChats.get(userId).unread = true;
    }
}

function clearUnreadIndicator(userId) {
    const indicator = document.querySelector(`[data-user-id="${userId}"] .unread-indicator`);
    if (indicator) indicator.style.display = "none";
    if (privateChats.has(userId)) {
        privateChats.get(userId).unread = false;
    }
}
...
async function uploadAvatarToFreeImage(file) {
    try {
        ...
        const response = await fetch("https://freeimage.host/api/1/upload?key=" + FREEIMAGE_API_KEY, { method: "POST", body: formData });
...
function setupAvatarModal() {
    ...
    avatarBtn.addEventListener("click", () => { avatarModal.classList.add("active"); });
...
async function initializePeerConnection() {
    try {
        ...
        peerConnection = new Peer({ host: PEERJS_HOST, port: PEERJS_PORT, path: PEERJS_PATH, secure: true, key: PEERJS_KEY });
...
async function registerPeerInSupabase(peerId) {
    try {
        ...
        const { error } = await supabase.from("active_users").upsert({ username: currentUsername, peer_id: peerId, room_id: currentRoom, updated_at: new Date().toISOString() }, { onConflict: "username" });
        if (error) throw error;
...
async function subscribeToActiveUsers() {
    try {
        ...
        activeUsersChannel.on("postgres_changes", { event: "*", schema: "public", table: "active_users" }, async (payload) => {
...
async function updateActiveUsersList() {
    try {
        const { data, error } = await supabase.from("active_users").select("*").eq("room_id", currentRoom);
        if (error) throw error;
        ...
        data.forEach((user) => {
            ...
            badge.className = "active-user-badge cursor-pointer hover:bg-blue-500 hover:bg-opacity-40";
            badge.innerHTML = `<span>👤</span><span>${user.username}</span><span class="text-xs text-gray-400">(${user.peer_id.substr(0, 6)}...)</span>`;
            badge.onclick = () => openPrivateChat(user.peer_id, user.username);
...
async function getUserMediaStream() {
    try {
        ...
        const stream = await navigator.mediaDevices.getUserMedia({ audio: voiceEnabled, video: cameraEnabled ? { width: 640, height: 480 } : false });
        console.log("[MEDIA] Stream obtained successfully");
        userLocalStream = stream;
        if (cameraEnabled) displayLocalVideoPreview(stream);
        return stream;
    } catch (error) {
...
async function initiateWebRTCCalls() {
    try {
        ...
        if (!stream) return;
        const { data: activeUsers, error } = await supabase.from("active_users").select("*").eq("room_id", currentRoom);
        if (error) throw error;
...
function displayMessage(messageData) {
    ...
    // Mark private chat as unread if not active
    if (currentPrivateChat && messageData.room_id === `private_${currentPrivateChat}` && !document.querySelector(`[data-user-id="${currentPrivateChat}"]`)) {
        markPrivateChatUnread(currentPrivateChat);
    }
