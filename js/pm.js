...
async function ensurePmRealtime() {
  if (!currentUser?.id || (pmWs && (pmWs.readyState === WebSocket.OPEN || pmWs.readyState === WebSocket.CONNECTING))) return;

  pmWs = new WebSocket(`wss://${location.host}/api/ws/pm?token=${encodeURIComponent(getToken() || '')}`);
  pmWs.onmessage = async (event) => {
    let msg;
    try { msg = JSON.parse(event.data); } catch (_) { return; }
    if (msg.type === 'pm') await handleIncomingPm(msg);
  };
  pmWs.onclose = () => { pmWs = null; };
}
...
async function openPrivateChat(userId, username) {
  ...
  input.onkeydown = (e) => { if (e.key === 'Enter') sendPrivateText(userId); };
...
function closePrivateChat(userId) {
  ...
  if (pmRecorders[userId]?.state === 'recording') pmRecorders[userId].stop();
...
function positionPmWindows() {
  Object.values(pmWindows).forEach((win, idx) => {
...
async function sendPrivateText(userId) {
  ...
  appendPmTextMessage(userId, { from: currentUser.id, text, createdAt }, true);

  if (!pmTextHistory[userId]) pmTextHistory[userId] = [];
  pmTextHistory[userId].push({ from: currentUser.id, text, createdAt });

  await ensurePmRealtime();
  if (pmWs?.readyState === WebSocket.OPEN) {
    pmWs.send(JSON.stringify({ type: 'pm', to: userId, msgType: 'text', text, createdAt }));
  }
}
...
async function handleIncomingPm(payload) {
  ...
  if (payload.msgType === 'voice' && payload.voiceDataUrl) {
...
function renderPmTextHistory(userId) {
  ...
  if (!box) return;
  box.innerHTML = '';
  (pmTextHistory[userId] || []).forEach((msg) => appendPmTextMessage(userId, msg, msg.from === currentUser.id));
}
...
async function togglePmRecording(userId) {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      await ensurePmRealtime();
      if (pmWs?.readyState === WebSocket.OPEN) {
        pmWs.send(JSON.stringify({ type: 'pm', to: userId, msgType: 'voice', voiceDataUrl, createdAt: new Date().toISOString() }));
      }
    };
...
async function loadPmHistoryFromDb(userId) {
  const res = await apiFetch(`/api/pm/history/${encodeURIComponent(userId)}`);
  if (!res.ok) return;
