...
function closePrivateChat(userId) {
  ...
  if (pmRecorderTimers[userId]) {
    clearTimeout(pmRecorderTimers[userId]);
    delete pmRecorderTimers[userId];
  }
...
async function togglePmRecording(userId) {
  ...
  try {
    ...
    mediaRecorder.start();
    pmRecorderTimers[userId] = setTimeout(() => {
